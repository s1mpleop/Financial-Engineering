...
def solve(t,S_at_0,K,T,M,r,Sigma):
    ...
    for k in range(0, M+1):
        S= S_at_0*math.pow(u,k)*math.pow(d,M-k)
            
        S1= max(0, S-K)
        S1= S1*math.pow(pStar,k)*math.pow(qStar,M-k)
        S1= S1*nCr(M,k)
        E_Call_Price= E_Call_Price+S1

        S2= max(0, K-S)
        S2= S2*math.pow(pStar,k)*math.pow(qStar,M-k)
        S2= S2*nCr(M,k)
        E_Put_Price= E_Put_Price+S2
                
    E_Call_Price= E_Call_Price/math.pow(math.exp(r*Delta_t),M)
    E_Put_Price= E_Put_Price/math.pow(math.exp(r*Delta_t),M)
    
    return [E_Call_Price,E_Put_Price]
...
print("Value of European Call Option for set 1: ",(Price[0]))
print("Value of European Putt Option for set 1: ",(Price[1]))
print("\n")
...
for Sigma_Graph in range(1000, 2000, 5):
    Price= solve(1,S_at_0,K,T,M,r,Sigma_Graph/10000.00)
    E_Call.append(Price[0])
    E_Put.append(Price[1])
    Sigma_G.append(Sigma_Graph/10000.00)
...
print("Value of European Call Option for set 2: ",(Price[0]))
print("Value of European Putt Option for set 2: ",(Price[1]))
print("")
